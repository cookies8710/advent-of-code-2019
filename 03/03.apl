...
⍝ todo better approach - gen line segments, compute intersections (no drawing)
...
∇result←solve wires
  ...
  sum ← ⊃⊃+/ {draw ⍵ ((⊂0 0),(+\⍵))}¨ vectors ⍝ todo why ⊃⊃

  ⍝ intersections
  results←((DIM*2)⍴(2 ⍷ sum)) / ⍳DIM*2 
  ⍝ compute manhatan distance from origin
  manh←{((⌊⍵÷DIM)-origin[0])+((DIM|⍵)-origin[1])} results
