...
∇result←solve wires;vectors;paths;a;b
...
∇result ← palens data;a;b;i1;i2;I;va;vb;l1;l2;r
 a ← ⊃data[0]
 b ← ⊃data[1]
 i1 ← data[2]
 i2 ← data[3]
...
∇result←solve2 wires;vectors;paths;a;b
⍝ parse wires to vectors
vectors ← parse¨ wires
...
veclen ← {+/|⍵}
pathlen ← {+/ (veclen¨ (⍵[0])↑ ⊃⍵[1])}
'pathlen', pathlen 3 (⊃vectors[0])
⍝{pathlen ⍵[0] (⊃vectors[0])}¨ its
p0←⊃vectors[0]
p1←⊃vectors[1]
4⎕CR2↑p0
'kk'
⍝{+/ (veclen¨ (⍵[0])↑ (0 0) (1 1)])}¨its
4⎕CR⊃paths[0]
{(+/veclen¨|⍵[0]↑p0) + (+/veclen¨|⍵[1]↑p1) }¨ its
⍝ + (veclen paths[0])

⍝ part2: sum complete vectors + size(intersection - last point in path)
'PATHS'
4⎕CRa
4⎕CRb

⍝ working: i1, i2 are partial wire lengths, I is intersection
i1←2
i2←2
I←6 5
'((i1+1)↑PATH),⊂(V 2)'
...
'r=',r
palens a b 3 3 (3 3)
4⎕CRpalens a b (3⍴⊃its[2])[0] (3⍴⊃its[2])[1] ((3⍴⊃its[2])[2])

+/ (veclen¨ 3↑ ⊃vectors[0])
+/ (veclen¨ 3↑ ⊃vectors[1])

{+/ (veclen¨ (⍵[0])↑ ⊃vectors[⍵[1]])} 3 0
{+/ (veclen¨ (⍵[0])↑ ⊃⍵[1])} 3 (⊃vectors[0])
